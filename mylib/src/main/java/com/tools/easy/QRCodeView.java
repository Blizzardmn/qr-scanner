...
public abstract class QRCodeView extends RelativeLayout implements Camera.PreviewCallback {
    ...
    public void onPreviewFrame(final byte[] data, final Camera camera) {
        if (QRCodeUtils.isDebug()) {
            QRCodeUtils.d("两次 onPreviewFrame 时间间隔：" + (System.currentTimeMillis() - mLastPreviewFrameTime));
        ...
        mProcessDataTask = new ProcessDataTask(camera, data, this, QRCodeUtils.isPortrait(getContext())).perform();
    }
    ...
    private void handleAmbientBrightness(byte[] data, Camera camera) {
        ...
        if (Math.abs(data.length - pixelCount * 1.5f) < 0.00001f) {
            ...
            QRCodeUtils.d("摄像头环境亮度为：" + cameraLight);
            if (mDelegate != null) {
     ...
     */
    public void decodeQRCode(Context context, Uri uri) {
        mProcessDataTask = new ProcessDataTask(context, uri, this).perform();
    }
    ...
    protected abstract BarcodeResult processData(byte[] data, int width, int height, int imageFormat, boolean isRetry);

    protected abstract BarcodeResult processBitmapData(Bitmap bitmap);

    void onPostParseData(BarcodeResult scanResult) {
        if (!mSpotAble) {
            return;
        }
        String result = scanResult == null ? null : scanResult.getResult().getText();
    ...
    void onPostParseBitmapOrPicture(BarcodeResult result) {
        if (mDelegate != null) {
    ...
    protected boolean transformToViewCoordinates(final PointF[] pointArr, final Rect scanBoxAreaRect, final boolean isNeedAutoZoom, final BarcodeResult result) {
        ...
        try {
            ...
            int statusBarHeight = QRCodeUtils.getStatusBarHeight(getContext());
    ...
    private boolean handleAutoZoom(PointF[] locationPoints, final BarcodeResult result) {
    ...
    private void startAutoZoom(int oldZoom, int newZoom, final BarcodeResult result) {
    ...
    private PointF transform(float originX, float originY, float cameraPreviewWidth, float cameraPreviewHeight, boolean isMirrorPreview, int statusBarHeight,
        ...
        if (QRCodeUtils.isPortrait(getContext())) {
    ...
    public interface Delegate {
         ...
         */
        void onScanQRCodeSuccess(BarcodeResult result);
